<div>
    <style>
        ...
        .kcms-login-forgot a { color: #059669; text-decoration: none; }
    ...
    <div class="kcms-login-wrap">
        ...
        <div class="kcms-login-form-col">
            <div class="kcms-login-card">
                ...
                <div class="kcms-login-heading">
                    ...
                    <h1>{{ __('admin_login.form.heading') }}</h1>
                    {{-- <p>{{ __('admin_login.form.subheading') }}</p> --}}
                </div>

                <form wire:submit="authenticate">
                    {{ $this->form }}
                    <button type="submit" class="kcms-login-submit">
                        {{ __('admin_login.form.submit') }}
                    </button>
                </form>

                @if (Illuminate\Support\Facades\Route::has('filament.admin.auth.password-reset.request'))
                    <div class="kcms-login-forgot">
                        <a href="{{ route('filament.admin.auth.password-reset.request') }}">
                            {{ __('admin_login.form.forgot_password') }}
                        </a>
                    </div>
                @endif
        ...
        <div class="kcms-login-panel-col">
            <div class="kcms-login-panel-inner">
                <h2>{{ __('admin_login.panel.heading') }}</h2>
                <p>{{ __('admin_login.panel.subheading') }}</p>

                <ul class="kcms-feature-list">
                    @foreach (__('admin_login.panel.features') as $feature)
